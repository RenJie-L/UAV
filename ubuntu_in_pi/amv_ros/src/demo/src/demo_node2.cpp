...
int main(int argc, char **argv)
{
   ...
   // double dz[2][2]={{0,0},{0.3,0}};
    ...
    double dx[2][2]={{0,0},{1,0}};
    double dy[2][2]={{0,0},{1,0}};
    double dz[2][2]={{0,0},{1,0}};
    ...
    while(ros::ok()){
        ...
        else
        {
            ...
            {    
                ...
                if(epock>=300&&epock<600)
                   {x[0]=0;
                     y[0]=0.5;
                     z[0]=0.3;
                     }
                 if(epock>=600&&epock<900)
                   {x[0]=-0.5;
                     y[0]=0;
                     z[0]=0.3;
                     }
...
/*
//*
double clcu_theta(vector<double> P_r,vector<double> P_c,double theta)
{
double out;
out=atan2(P_c[1]-P_r[1],P_c[0]-P_r[0])-theta;
return out;
}
//
double clcu_phi(double dist,vector<double> P_n,vector<double> P_i)
{
double out;
double num_a=(80/180.0)*pi;
double const_num=(80/180.0)*pi;
if(dist<=d_a)
{
out=1;
}else
  {double theta=clcu_theta(P_n,P_i,P_n[2]);
	if(abs(theta)<=num_a&&dist>d_a)
         {out=cos(theta)*cos(theta);
 
}else
{out=cos(const_num)*cos(const_num)
}
}
return out;
}
//
int clcu_nowmju(vector<double> P_n,vector <double> P_i)
{
double dist_i,phi;
int out;
dist_i=clcu_dist(P_n,P_i);
phi=clcu_phi(dist_i,P_n,P_i);
out=clcu_mju(dist_i,phi);
return out;
}
//
double clcu_mju(double dist,double phi)
{
double c=0.000001;
double beta=2.13;
double ot;
out =10*(log10(c)-beta*log10(dist)+log10(phi));
return out;
}
//
double clcu_Gi(double Z,double mju)
{
double sigma=3.3;
double alfa,out;
alfa=1/(sqrt(2)*sigma);
out=0.5*erfc(-(Z-mju)*alfa);
return out;
}
double clcu_dist(vector<double> P_r,vector<double> P_c)
{
double out;
out=sqrt((P_r[0]-P_c[0])*(P_r[0]-P_c[0])+(P_r[1]-P_c[1])*(P_r[1]-P_c[1]));
return out;
}
//
...
double NormalizeEval(vector<double> EvalDB)
{if (sum(EvalDB(:,3))!=0)
	{EvalDB(:,3)=EvalD(:,3)/sum(EvalDB(:,3))';
          }
if (sum(EvalDB(:,4))!=0)
	{EvalDB(:,4)=EvalD(:,4)/sum(EvalDB(:,4))';
          }
if (sum(EvalDB(:,5))!=0)
	{EvalDB(:,5)=EvalD(:,5)/sum(EvalDB(:,5
  ))';
          }
return EvalDB;
}*//
