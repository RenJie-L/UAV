...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber optitrack_sub = nh.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node/Trackable4/pose", 100, optitrack_cb);
            ...
            {  
                 if (epock<600)
                    {/*double goal_x=1;
                     ...
                     double start_x=-1.2;
                     double start_y=-1.2;
                     double jiaodu=atan((goal_y-start_y)/(goal_x-start_x));
                     double k=(goal_x-start_x)/cos(jiaodu*180/PI);
                     
                     x[1]=start_x+cos(jiaodu*180/PI)*k*(epock)/600;
                     y[1]=start_y+sin(jiaodu*180/PI)*k*(epock)/600;
                     z[1]=0.3;*/
                     x[1]=-0.7;
                     y[1]=0.25;
                     z[1]=0.3;
                ...
                x[1] = x[1] + uxx[1] * 0.01*cos(Euler_fcu[2]/PI*180);

                uxy[1] =  vy[1] - beta * sign2 * pow(abs(sumsum2),0.3333);
                y[1] = y[1] + uxy[1] * 0.01*sin(Euler_fcu[2]/PI*180);
