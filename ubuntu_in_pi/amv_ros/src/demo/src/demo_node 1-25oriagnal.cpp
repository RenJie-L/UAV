...
float l,float data_noise)
{
	...
	Eigen::MatrixXd k_X_test_train = kernel_matrix_f2vect(X_train,X_test,g,l);
        ...
        Eigen::MatrixXd mean;
	mean = m_test+k_X_test_train_transpose * K_X_train_inv * (Y_train);//(TxN)(NxN)(NxD) = (TxD) 
	//mean = k_X_test_train; 
        return mean;
...
int ret= pthread_create(&p, NULL,th_fun,NULL);//  
  ...
  cout << "RHC start!!" << endl;
	...
	float Kinematic[6] = {0.1, DegreeToRadian(80.0), 2,DegreeToRadian(50.0), 0.01, DegreeToRadian(0.1) };
	Point end;
        end.x=0;
        end.y=0;
 ...
 /*int ret= pthread_create(&p, NULL,th_fun,NULL);//  
...

    {
		...
		if (tdist < 0.2)
		{
			...
			ROS_INFO_STREAM("predic.x1, predic.y1: "<<X_temp<<","<<Y_temp);
                    temp.clear();
                    int read_rssi;
