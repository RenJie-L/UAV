...
{       
   ...
   /* pthread_t p;
    int ret = pthread_create(&p, NULL,  read_fun, NULL);//
    if(ret != 0)
    {
  ...
  cout << "RHC start!!" << endl;
	...
	float Kinematic[6] = {0.6, DegreeToRadian(20.0), 2,DegreeToRadian(50.0), 0.02, DegreeToRadian(0.1) };
	Point end;
       // end.x=1.2;
        //end.y=1.2;
...
//while(ros::ok){
  ...
  while(ros::ok()){
        if (current_state.mode != "OFFBOARD" &&
                (ros::Time::now() - last_request > ros::Duration(3.0))&&epock<=5) {
